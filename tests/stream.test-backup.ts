...
beforeAll(async () => {
  // Initialize simnet with the project path
  const projectPath = path.resolve(process.cwd());
  simnet = await initSimnet(projectPath);
...
describe("Token Streaming Contract Tests", () => {
  ...
  it("ensures recipient can withdraw tokens over time", () => {
    ...
    const transferEvent = withdrawal.events.find((e: any) => e.event === "stx_transfer_event");
  ...
  it("ensures sender can refund excess tokens after stream ends", () => {
    ...
    const transferEvent = refund.events.find((e: any) => e.event === "stx_transfer_event");
    expect(transferEvent).toBeDefined();
